...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents) <= self.volume:
            leftover = self.volume - len(self.contents)
            for k in range(leftover):
                self.contents = self.contents + str([additional_contents[k]])
            if leftover >= len(additional_contents):
                for k in range(leftover, len(additional_contents)):
                    s = s + str([additional_contents[k]])
